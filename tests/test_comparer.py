...
def test_numeric_precision_and_boolean_equivalence_avoid_false_discrepancies(tmp_path) -> None:
    left_path = tmp_path / "left.csv"
    right_path = tmp_path / "right.csv"
    write_csv(
        left_path,
        ["customer_id", "transaction_date", "amount", "status", "description"],
        [
            {"customer_id": "C1", "transaction_date": "2026-01-01", "amount": "14.3553", "status": "1", "description": "alpha"},
        ],
    )
    write_csv(
        right_path,
